/*
    DESCRIPTION:
    Microsoft Windows 11 variables using the Packer Builder for Nutanix (Prism Central).
*/
...
variable "vm_cpu_sockets" {
  type        = number
  description = "The number of virtual CPUs sockets. (e.g. '4')"
  default     = 4
}
...
variable "vm_disk_size" {
  type        = number
  description = "The size for the virtual disk in GB. (e.g. '60')"
  default     = 60
}

variable "vm_vtpm" {
  type        = bool
  description = "Enable virtual trusted platform module (vTPM). Required by Windows 11."
  default     = true
}
...
variable "iso_uri" {
  type        = string
  description = "The URI of the guest operating system ISO. Downloaded into the Prism image library on first use. (e.g. 'http://10.0.0.6:8080/Microsoft/windows_11.iso')"
}
...
variable "virtio_driver_path" {
  type        = string
  description = "The driver folder on the VirtIO ISO for this operating system. (e.g. 'E:\\Windows 11\\x64')"
  default     = "E:\\Windows 11\\x64"
}
...
variable "vm_shutdown_command" {
  ...
  // Sysprep runs through a SYSTEM scheduled task: processes spawned directly
  // in the WinRM session are killed when Packer closes the session (they
  // belong to its job object), and running sysprep synchronously leaves
  // Packer waiting on a WinRM reply that never arrives. The task fires once,
  // outlives the session, and generalizes then powers off the VM.
  default     = "schtasks /Create /F /RU SYSTEM /RL HIGHEST /SC ONCE /ST 00:00 /TN PackerSysprep /TR \"C:\\Windows\\System32\\Sysprep\\sysprep.exe /generalize /oobe /shutdown /quiet\" && schtasks /Run /TN PackerSysprep"
}
